...
namespace REDFLAG
{
    public partial class Form8 : Form
    {
        ...
        {
            connectionString = "Server=localhost;Database=authorization_registration;User ID=root;Password=fs!if3nj#wkr32jrf/9/329fjwfgsgf3t@r23#8t32trh32ht2!3@ht832532";
            connection = new MySqlConnection(connectionString);
            connection.Open();

            // Create a command to add data to the table
            string query = "INSERT INTO authorization_registration.employees (id, full_name, positions_id, clinics_id) " +
            ...
            command.Parameters.AddWithValue("@positions_id", int.Parse(textBox3.Text));
            command.Parameters.AddWithValue("@clinics_id", int.Parse(textBox4.Text));

            // Execute the command
            command.ExecuteNonQuery();

            // Update the data in DataGridView
            string selectQuery = "SELECT * FROM authorization_registration.employees";
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            using (MySqlCommand cmd = new MySqlCommand("DELETE FROM employees WHERE id = @id", conn))
            {
